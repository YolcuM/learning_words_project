...
const useValidation = () => {
    ...
    function validateField (nameField, valueField){
        if(valueField === ''){
            ...
            setImportErrorText({...inputErrorText, [nameField]:'Заполните поле'})
        }
        else{
            switch(nameField){
                case 'english':
                    if(valueField.match(/^[а-яА-ЯёЁ]+$/)){
                        setIsInputError({...isInputError, [nameField]:true})
                        setImportErrorText({...inputErrorText, [nameField]:'Допускаются только английские буквы'})
                    }
                    else{
                ...
                case 'transcription':
                    ...
                    if(valueField.match(/^[a-zA-Z]+$/)){
                        setIsInputError({...isInputError, [nameField]:true})
                        setImportErrorText({...inputErrorText, [nameField]:'Допускаются только русские буквы'})
                    }
                    else{
                        setIsInputError({...isInputError, [nameField]:false})
                        setImportErrorText({...inputErrorText, [nameField]:''})
